<?php
// iScreendflog.php  (FULL FILE)
// DFlog Viewer (CSV Logs) — list/view/download/delete + bulk delete
// ✅ JS แยกไฟล์: assets/js/iScreendflog.js
session_start();
if (empty($_SESSION['UserID']) || !isset($_SESSION['userLevel'])) {
  echo("<script>location.href = 'login.php';</script>");
  exit();
}
$userID = $_SESSION['UserID'];
$userLevelAdmin = $_SESSION['userLevel'];
$userName = $_SESSION['userName'] ?? "";

// =========================================================
// CONFIG
// =========================================================
$logBaseDir = "/var/www/html/log/iScreenDF";
$validExt   = ['csv'];
$perPage    = 24;
$currentScript = basename(__FILE__);

// =========================================================
// HELPERS
// =========================================================
function h($s): string {
  return htmlspecialchars((string)$s, ENT_QUOTES, "UTF-8");
}

function fileExt(string $file): string {
  return strtolower(pathinfo($file, PATHINFO_EXTENSION));
}

function isValidLogName(string $name, array $validExts): bool {
  if ($name === "") return false;
  $ext = fileExt($name);
  return in_array($ext, $validExts, true);
}

// ป้องกัน path traversal: รับ relative path ภายใต้ base dir เท่านั้น
function safeRealPathUnder(string $baseDir, string $relative): ?string {
  $baseReal = realpath($baseDir);
  if (!$baseReal) return null;

  $relative = str_replace(["\0"], "", $relative);
  $relative = ltrim($relative, "/\\");
  if ($relative === "") return null;

  // ห้ามมี .. แบบลัด
  if (strpos($relative, "..") !== false) return null;

  $candidate = $baseReal . DIRECTORY_SEPARATOR . $relative;
  $real = realpath($candidate);
  if (!$real) return null;

  // Must be under base
  if (strpos($real, $baseReal . DIRECTORY_SEPARATOR) !== 0) return null;
  if (!is_file($real)) return null;

  return $real;
}

function relPathFromBase(string $baseDir, string $absPath): string {
  $baseReal = realpath($baseDir);
  $absReal  = realpath($absPath);
  if (!$baseReal || !$absReal) return "";

  $prefix = $baseReal . DIRECTORY_SEPARATOR;
  if (strpos($absReal, $prefix) === 0) {
    return substr($absReal, strlen($prefix));
  }
  return "";
}

function fmtBytes($bytes): string {
  $b = (float)$bytes;
  if ($b < 1024) return number_format($b, 0) . " B";
  if ($b < 1024*1024) return number_format($b/1024, 1) . " KB";
  if ($b < 1024*1024*1024) return number_format($b/(1024*1024), 1) . " MB";
  return number_format($b/(1024*1024*1024), 2) . " GB";
}

function parseCsvPreview(string $filePath, int $maxRows = 600): array {
  $res = ['headers'=>[], 'rows'=>[], 'err'=>""];
  if (!is_readable($filePath)) { $res['err']="File not readable."; return $res; }

  $fh = @fopen($filePath, "r");
  if (!$fh) { $res['err']="Open failed."; return $res; }

  $headers = fgetcsv($fh);
  if (!is_array($headers) || count($headers) === 0) {
    fclose($fh);
    $res['err']="CSV header missing/invalid.";
    return $res;
  }

  $headers = array_map(fn($x) => trim((string)$x), $headers);
  $res['headers'] = $headers;

  $count = 0;
  while (!feof($fh) && $count < $maxRows) {
    $row = fgetcsv($fh);
    if ($row === false) break;
    if (!is_array($row)) continue;

    if (count($row) < count($headers)) {
      $row = array_pad($row, count($headers), "");
    } else if (count($row) > count($headers)) {
      $row = array_slice($row, 0, count($headers));
    }
    $res['rows'][] = $row;
    $count++;
  }
  fclose($fh);
  return $res;
}

function buildQuery(array $over = []): string {
  $base = $_GET;
  foreach ($over as $k=>$v) {
    if ($v === null) unset($base[$k]);
    else $base[$k] = $v;
  }
  return http_build_query($base);
}

// ✅ true ถ้า directory ว่าง (ไม่มีไฟล์/โฟลเดอร์ย่อย)
function isDirEmpty(string $dir): bool {
  if (!is_dir($dir)) return false;
  $items = @scandir($dir);
  if (!is_array($items)) return false;
  foreach ($items as $it) {
    if ($it === '.' || $it === '..') continue;
    return false;
  }
  return true;
}

// ✅ ลบโฟลเดอร์ที่ว่าง “ไล่ขึ้นไป” จนถึง baseDir (ไม่ลบ base เอง)
function pruneEmptyDirsUpToBase(string $baseDir, string $fileAbsPath): void {
  $baseReal = realpath($baseDir);
  if (!$baseReal) return;

  $fileReal = realpath($fileAbsPath);
  // หลัง unlink realpath(file) อาจเป็น false -> ใช้ dirname ที่ส่งมาแทน
  $startDir = dirname($fileAbsPath);

  $dir = realpath($startDir) ?: $startDir;
  $basePrefix = rtrim($baseReal, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;

  // ต้องอยู่ใต้ base เท่านั้น
  $dirReal = realpath($dir);
  if ($dirReal && strpos($dirReal . DIRECTORY_SEPARATOR, $basePrefix) !== 0) return;

  while (true) {
    $dirReal = realpath($dir);
    if (!$dirReal) break;

    // หยุดถ้าถึง base หรืออยู่นอก base
    if ($dirReal === $baseReal) break;
    if (strpos($dirReal . DIRECTORY_SEPARATOR, $basePrefix) !== 0) break;

    // ลบได้เฉพาะถ้าว่าง
    if (!isDirEmpty($dirReal)) break;

    // ลบ dir นี้
    @rmdir($dirReal);

    // ขึ้นไป parent
    $parent = dirname($dirReal);
    if ($parent === $dirReal) break;
    $dir = $parent;
  }
}

// =========================================================
// INPUT
// =========================================================
$relFile = "";
$action  = "";

if (isset($_GET['file'])) {
  $relFile = trim(urldecode((string)$_GET['file']));
  $relFile = ltrim($relFile, "/\\");
}
if (isset($_GET['action'])) {
  $action = (string)$_GET['action'];
}

$q    = trim((string)($_GET['q'] ?? ""));
$freq = trim((string)($_GET['freq'] ?? ""));
$day  = trim((string)($_GET['day'] ?? ""));

$page = max(1, (int)($_GET['page'] ?? 1));
$offset  = ($page - 1) * $perPage;

// =========================================================
// STATUS MESSAGES
// =========================================================
$flashOk  = "";
$flashErr = "";

// =========================================================
// DELETE HANDLER (POST)
// =========================================================
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $postAction = $_POST['action'] ?? "";

  if ($postAction === "delete_one") {
    $name = trim((string)($_POST['file'] ?? ""));
    $name = ltrim($name, "/\\");

    if (!isValidLogName(basename($name), $validExt)) {
      $flashErr = "Invalid file name/type.";
    } else {
      $real = safeRealPathUnder($logBaseDir, $name);
      if (!$real) {
        $flashErr = "File not found.";
      } else {
        if (@unlink($real)) {
          // ✅ ลบไฟล์แล้ว -> prune folders ที่ว่าง
          pruneEmptyDirsUpToBase($logBaseDir, $real);
          $flashOk = "Deleted: " . $name;
        } else {
          $flashErr = "Delete failed (permission?).";
        }
      }
    }

  } else if ($postAction === "delete_selected") {
    $files = $_POST['files'] ?? [];
    if (!is_array($files) || count($files) === 0) {
      $flashErr = "No files selected.";
    } else {
      $deleted = 0;
      $failed  = 0;

      foreach ($files as $f) {
        $name = trim((string)$f);
        $name = ltrim($name, "/\\");
        if (!isValidLogName(basename($name), $validExt)) { $failed++; continue; }

        $real = safeRealPathUnder($logBaseDir, $name);
        if (!$real) { $failed++; continue; }

        if (@unlink($real)) {
          // ✅ ลบไฟล์แล้ว -> prune folders ที่ว่าง
          pruneEmptyDirsUpToBase($logBaseDir, $real);
          $deleted++;
        } else {
          $failed++;
        }
      }

      if ($deleted > 0) $flashOk = "Deleted $deleted file(s).";
      if ($failed  > 0) $flashErr = ($flashErr ? ($flashErr . " ") : "") . "Failed $failed file(s).";
    }
  }
}

// =========================================================
// DOWNLOAD (single csv)
// =========================================================
if ($action === "download" && $relFile !== "") {
  if (!isValidLogName(basename($relFile), $validExt)) {
    header("HTTP/1.0 404 Not Found");
    echo "Log not found or invalid file type.";
    exit();
  }

  $real = safeRealPathUnder($logBaseDir, $relFile);
  if (!$real) {
    header("HTTP/1.0 404 Not Found");
    echo "Log not found or invalid file type.";
    exit();
  }

  header('Content-Description: File Transfer');
  header('Content-Type: text/csv');
  header('Content-Disposition: attachment; filename="' . basename($relFile) . '"');
  header('Content-Length: ' . filesize($real));
  header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
  header('Pragma: no-cache');

  readfile($real);
  exit();
}

// =========================================================
// MODE: viewer or list
// =========================================================
$showError    = false;
$errorMsg     = "";
$isViewerMode = ($relFile !== "");

// list
$logs = [];
$total = 0;

// viewer data
$viewer = null;

$baseReal = realpath($logBaseDir);
if (!$baseReal) {
  $showError = true;
  $errorMsg  = "Log base directory not found: " . $logBaseDir;
} else {
  if ($isViewerMode) {
    if (!isValidLogName(basename($relFile), $validExt)) {
      $showError = true;
      $errorMsg  = "Log not found or invalid file type.";
    } else {
      $real = safeRealPathUnder($logBaseDir, $relFile);
      if (!$real) {
        $showError = true;
        $errorMsg  = "Log not found or invalid file type.";
      } else {
        $stat = [
          'mtime' => @filemtime($real) ?: 0,
          'size'  => @filesize($real) ?: 0,
        ];
        $csv = parseCsvPreview($real, 600);
        $viewer = [
          'rel' => $relFile,
          'abs' => $real,
          'mtime' => $stat['mtime'],
          'size'  => $stat['size'],
          'csv'   => $csv,
        ];
      }
    }
  } else {
    // Scan logs (recursive)
    $all = [];
    $it = new RecursiveIteratorIterator(
      new RecursiveDirectoryIterator($baseReal, FilesystemIterator::SKIP_DOTS)
    );

    $qLower    = strtolower($q);
    $freqLower = strtolower($freq);
    $dayLower  = strtolower($day);

    foreach ($it as $fileInfo) {
      /** @var SplFileInfo $fileInfo */
      if (!$fileInfo->isFile()) continue;

      $name = $fileInfo->getFilename();
      if (!isValidLogName($name, $validExt)) continue;

      $abs = $fileInfo->getPathname();
      $rel = relPathFromBase($logBaseDir, $abs);
      if ($rel === "") continue;

      $relLower = strtolower($rel);

      if ($qLower !== "" && strpos($relLower, $qLower) === false) continue;
      if ($freqLower !== "" && strpos($relLower, $freqLower) === false) continue;
      if ($dayLower !== "" && strpos($relLower, $dayLower) === false) continue;

      $mtime = $fileInfo->getMTime();
      $size  = $fileInfo->getSize();

      $all[] = [
        'rel'  => $rel,
        'name' => $name,
        'mtime'=> $mtime,
        'size' => $size,
      ];
    }

    usort($all, fn($a,$b) => $b['mtime'] <=> $a['mtime']);

    $total = count($all);
    $logs  = array_slice($all, $offset, $perPage);
  }
}
?>
<!doctype html>
<html lang="en" data-bs-theme="auto">
<head>
  <script src="assets/js/color-modes.js"></script>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>IFZ: DFlog Viewer</title>
  <link href="assets/dist/css/bootstrap.min.css" rel="stylesheet">
  <link href="dashboard.css" rel="stylesheet">
  <script src="assets/dist/js/bootstrap.bundle.min.js"></script>

  <!-- ✅ external JS -->
  <script defer src="iScreendflog.js?v=<?php echo time();?>"></script>
    <script src="https://cdn.jsdelivr.net/npm/mgrs@1.0.0/dist/mgrs.min.js"></script>
  <style>
    .mono { font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono","Courier New", monospace; font-size: 0.88rem; }
    .sticky-actions { position: sticky; top: 64px; z-index: 50; background: var(--bs-body-bg); padding-top: 8px; padding-bottom: 8px; }
    .log-card { border-radius: 16px; overflow: hidden; }
    .table-wrap { overflow:auto; max-height: 70vh; border-radius: 12px; border: 1px solid rgba(125,125,125,0.25); }
  </style>
</head>

<header class="navbar sticky-top flex-md-nowrap p-0 shadow" style="background-color:#000000DD;">
  <svg class="bi_logo m-1"><use xlink:href="dashboard.svg?v=<?php echo time();?>#logo"></use></svg>
  <ul class="navbar-nav flex-row d-md-none">
    <li class="nav-item text-nowrap">
      <button class="nav-link px-3 text-white" type="button" data-bs-toggle="offcanvas" data-bs-target="#sidebarMenu">
        <svg class="bi"><use xlink:href="dashboard.svg?v=<?php echo time();?>#list"/></svg>
      </button>
    </li>
  </ul>
  <div id="navbarSearch" class="navbar-search w-100 collapse">
    <input class="form-control w-100 rounded-0 border-0" type="text" placeholder="Search" aria-label="Search">
  </div>
</header>

<body>
<div class="container-fluid">
  <div class="row">

    <!-- sidebar (คงของเดิมคุณ) -->
    <div class="sidebar border border-right col-md-3 col-lg-2 p-0 bg-body-tertiary">
      <div class="offcanvas-md offcanvas-end bg-body-tertiary" tabindex="-1" id="sidebarMenu" aria-labelledby="sidebarMenuLabel">
        <div class="offcanvas-body d-md-flex flex-column p-0 pt-lg-3 overflow-y-auto">
          <ul class="nav flex-column">
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" aria-current="page" href="index.php">
                <svg class="bi"><use xlink:href="dashboard.svg#house-fill"/></svg>
                Home
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="playRecording.php">
                <svg class="bi"><use xlink:href="fontawesome-free-5.15.4-web/sprites/solid.svg#record-vinyl" /></svg>
                Recorder Playback
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" aria-current="page" href="eventLoggerData.php">
                <svg class="bi"><use xlink:href="fontawesome-free-5.15.4-web/sprites/solid.svg?v=<?php echo time();?>#layer-group"/></svg>
                Event Logger
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" aria-current="page" href="audiofiles">
                <svg class="bi"><use xlink:href="fontawesome-free-5.15.4-web/sprites/regular.svg?v=<?php echo time();?>#file-audio"/></svg>
                Audio Archive
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" aria-current="page" href="mapvisual.php">
                <svg class="bi"><use xlink:href="dashboard.svg?v=<?php echo time();?>#mapvisual"/></svg>
                Map Visual
              </a>
            </li>
          </ul>

          <hr class="my-3">
          <h6 class="sidebar-heading d-flex justify-content-between align-items-center px-3 mt-4 mb-1 text-body-secondary text-uppercase">
            <span>Device Manager</span>
          </h6>

          <ul class="nav flex-column mb-auto">
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="user.php">
                <svg class="bi"><use xlink:href="fontawesome-free-5.15.4-web/sprites/regular.svg#user"/></svg>
                Users
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="showimg.php">
                <svg class="bi"><use xlink:href="dashboard.svg?v=<?php echo time();?>#picture" /></svg>
                Picture
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2 active" href="iScreendflog.php">
                <svg class="bi"><use xlink:href="dashboard.svg?v=<?php echo time();?>#logdf" /></svg>
                DFlog Viewer
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="dfrole.php">
                <svg class="bi"><use xlink:href="dashboard.svg?v=<?php echo time();?>#DFGroup"/></svg>
                DF Role Setting
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="dfdevice.php">
                <svg class="bi"><use xlink:href="dashboard.svg?v=<?php echo time();?>#DFDevice"/></svg>
                DF Device Settings
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="settings.php">
                <svg class="bi"><use xlink:href="dashboard.svg#gear-wide-connected"/></svg>
                Settings
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="controler.php">
                <svg class="bi"><use xlink:href="dashboard.svg?v=<?php echo time();?>#plus-circle"/></svg>
                Register Device
              </a>
            </li>
            <li class="nav-item">
              <a class="nav-link d-flex align-items-center gap-2" href="logout.php">
                <svg class="bi"><use xlink:href="dashboard.svg?v=<?php echo time();?>#door-closed"/></svg>
                <?php echo "Sign out(" . h($userName) . ")"; ?>
              </a>
            </li>
          </ul>

        </div>
      </div>
    </div>

    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-2 border-bottom">
        <h1 class="h2"><?php echo $isViewerMode ? "DF Log Viewer" : "DF Log Files"; ?></h1>

        <?php if ($isViewerMode): ?>
          <a href="<?php echo h($currentScript . "?" . buildQuery(['file'=>null,'action'=>null,'page'=>null])); ?>" class="btn btn-outline-secondary">
            <svg class="bi"><use xlink:href="dashboard.svg#arrow-left"/></svg>
            Back to List
          </a>
        <?php endif; ?>
      </div>

      <?php if ($flashOk): ?>
        <div class="alert alert-success alert-dismissible fade show" role="alert">
          <?php echo h($flashOk); ?>
          <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        </div>
      <?php endif; ?>
      <?php if ($flashErr): ?>
        <div class="alert alert-warning alert-dismissible fade show" role="alert">
          <?php echo h($flashErr); ?>
          <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        </div>
      <?php endif; ?>

      <?php if($showError): ?>
        <div class="alert alert-warning alert-dismissible fade show" role="alert">
          <strong>Warning!</strong> <?php echo h($errorMsg); ?>
          <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
        </div>
        <a href="javascript:history.back()" class="btn btn-secondary">
          <svg class="bi"><use xlink:href="dashboard.svg#arrow-left"/></svg>
          Back
        </a>

      <?php else: ?>

        <?php if ($isViewerMode && $viewer): ?>
          <!-- =======================
               SINGLE LOG VIEW
               ======================= -->
          <div class="card log-card shadow-sm">
            <div class="card-body">

              <div class="d-flex flex-wrap gap-2 align-items-center justify-content-between">
                <div>
                  <div class="mono text-body-secondary">File</div>
                  <div class="mono fw-bold"><?php echo h($viewer['rel']); ?></div>
                  <div class="text-body-secondary small mt-1">
                    <?php echo date("Y-m-d H:i:s", (int)$viewer['mtime']); ?>
                    · <?php echo h(fmtBytes((int)$viewer['size'])); ?>
                  </div>
                </div>

                <div class="d-flex gap-2 flex-wrap">
                  <a href="<?php echo h($currentScript . "?" . buildQuery(['action'=>"download"])); ?>"
                     class="btn btn-primary">
                    <svg class="bi"><use xlink:href="dashboard.svg#download"/></svg>
                    Download CSV
                  </a>

                  <form method="POST" class="d-inline" onsubmit="return window.DFLOG_confirmDeleteOne(this);">
                    <input type="hidden" name="action" value="delete_one">
                    <input type="hidden" name="file" value="<?php echo h($viewer['rel']); ?>">
                    <button type="submit" class="btn btn-danger">
                      <svg class="bi"><use xlink:href="dashboard.svg#trash"/></svg>
                      Delete
                    </button>
                  </form>

                  <a href="<?php echo h($currentScript . "?" . buildQuery(['file'=>null,'action'=>null,'page'=>null])); ?>"
                     class="btn btn-secondary">
                    <svg class="bi"><use xlink:href="dashboard.svg#arrow-left"/></svg>
                    Back
                  </a>
                </div>
              </div>

              <hr class="my-3">

              <?php if ($viewer['csv']['err']): ?>
                <div class="alert alert-warning"><?php echo h($viewer['csv']['err']); ?></div>
              <?php else: ?>
                <div class="table-wrap">
                  <table class="table table-sm table-striped mb-0">
                    <thead class="table-dark sticky-top">
                      <tr>
                        <?php foreach ($viewer['csv']['headers'] as $hh): ?>
                          <th class="mono"><?php echo h($hh); ?></th>
                        <?php endforeach; ?>
                      </tr>
                    </thead>
                    <tbody>
                    <?php
                    $headers = $viewer['csv']['headers'];

                    $idxMgrs = array_search('mgrs', $headers, true);
                    $hasMgrs = ($idxMgrs !== false);

                    foreach ($viewer['csv']['rows'] as $row):
                    ?>
                    <tr>
                    <?php foreach ($headers as $ci => $colName): ?>
                        <?php
                        $cell = (string)($row[$ci] ?? "");

                        if ($hasMgrs && $ci === $idxMgrs) {
                            echo '<td class="mono">'
                                . '<a href="#"'
                                . ' class="mgrs-link"'
                                . ' data-mgrs="' . h($cell) . '"'
                                . ' title="' . h($cell) . '">'
                                . h($cell) . ' <small class="text-body-secondary">↗</small>'
                                . '</a>'
                                . '</td>';
                            continue;
                        }

                        echo '<td class="mono">' . h($cell) . '</td>';
                        ?>
                    <?php endforeach; ?>
                    </tr>
                    <?php endforeach; ?>
                    </tbody>


                  </table>
                </div>
                <div class="text-body-secondary small mt-2">
                  Preview rows: <?php echo count($viewer['csv']['rows']); ?> (limited)
                </div>
              <?php endif; ?>

            </div>
          </div>

        <?php else: ?>
          <!-- =======================
               LOG LIST + FILTERS + BULK DELETE
               ======================= -->
          <form class="row g-2 align-items-end mb-3" method="GET">
            <div class="col-12 col-md-4">
              <label class="form-label">Search (path/name)</label>
              <input type="text" class="form-control" name="q" value="<?php echo h($q); ?>" placeholder="e.g. 120_000MHz or 2026-01-22">
            </div>
            <div class="col-6 col-md-3">
              <label class="form-label">Freq filter (contains)</label>
              <input type="text" class="form-control" name="freq" value="<?php echo h($freq); ?>" placeholder="e.g. 120_000MHz">
            </div>
            <div class="col-6 col-md-3">
              <label class="form-label">Day filter (contains)</label>
              <input type="text" class="form-control" name="day" value="<?php echo h($day); ?>" placeholder="e.g. 2026-01-22 or 22012026">
            </div>
            <div class="col-12 col-md-2 d-grid">
              <button class="btn btn-primary" type="submit">Filter</button>
            </div>
          </form>

          <?php if ($total <= 0): ?>
            <div class="alert alert-info">
              <!-- No CSV logs found in <span class="mono"><?php echo h($logBaseDir); ?></span> -->
                No CSV logs found 
            </div>
          <?php else: ?>

            <div class="sticky-actions border-bottom mb-3">
              <form id="bulkForm" method="POST" class="d-flex flex-wrap gap-2 align-items-center">
                <input type="hidden" name="action" value="delete_selected">

                <button type="button" class="btn btn-outline-secondary btn-sm" onclick="DFLOG_toggleAll(true)">Select All</button>
                <button type="button" class="btn btn-outline-secondary btn-sm" onclick="DFLOG_toggleAll(false)">Clear</button>

                <button type="submit" class="btn btn-danger btn-sm" onclick="return DFLOG_confirmBulk();">
                  <svg class="bi"><use xlink:href="dashboard.svg#trash"/></svg>
                  Delete Selected
                </button>

                <span class="text-body-secondary small ms-2">
                  Selected: <span id="selCount">0</span>
                  · Total: <?php echo (int)$total; ?>
                </span>
              </form>
            </div>

            <div class="row g-3">
              <?php foreach ($logs as $it): ?>
                <div class="col-12 col-md-6 col-lg-4">
                  <div class="card log-card h-100 shadow-sm">
                    <div class="card-body">

                      <div class="d-flex justify-content-between align-items-start gap-2">
                        <div class="form-check">
                          <input class="form-check-input logCheck"
                                 type="checkbox"
                                 name="files[]"
                                 value="<?php echo h($it['rel']); ?>"
                                 form="bulkForm"
                                 onchange="DFLOG_updateCount()">
                        </div>

                        <div class="flex-grow-1">
                          <div class="mono text-truncate" title="<?php echo h($it['rel']); ?>">
                            <?php echo h($it['rel']); ?>
                          </div>
                          <div class="text-body-secondary small mt-1">
                            <?php echo date("Y-m-d H:i:s", (int)$it['mtime']); ?>
                            · <?php echo h(fmtBytes((int)$it['size'])); ?>
                          </div>
                        </div>
                      </div>

                      <div class="mt-3 d-flex gap-2 flex-wrap">
                        <a class="btn btn-sm btn-primary"
                           href="<?php echo h($currentScript . "?" . buildQuery(['file'=>$it['rel'], 'action'=>null, 'page'=>null])); ?>">
                          View
                        </a>

                        <a class="btn btn-sm btn-outline-secondary"
                           href="<?php echo h($currentScript . "?" . buildQuery(['file'=>$it['rel'], 'action'=>"download", 'page'=>null])); ?>">
                          Download
                        </a>

                        <form method="POST" class="d-inline" onsubmit="return window.DFLOG_confirmDeleteOne(this);">
                          <input type="hidden" name="action" value="delete_one">
                          <input type="hidden" name="file" value="<?php echo h($it['rel']); ?>">
                          <button type="submit" class="btn btn-sm btn-danger">
                            Delete
                          </button>
                        </form>
                      </div>

                    </div>
                  </div>
                </div>
              <?php endforeach; ?>
            </div>

            <?php
              $pages = (int)ceil($total / $perPage);
              if ($pages > 1):
                $prev = max(1, $page - 1);
                $next = min($pages, $page + 1);
            ?>
              <nav class="mt-4">
                <ul class="pagination">
                  <li class="page-item <?php echo ($page <= 1) ? "disabled" : ""; ?>">
                    <a class="page-link" href="<?php echo h($currentScript . "?" . buildQuery(['page'=>$prev])); ?>">Prev</a>
                  </li>

                  <?php
                    $start = max(1, $page - 3);
                    $end   = min($pages, $page + 3);
                    if ($start > 1) {
                      echo '<li class="page-item"><a class="page-link" href="'.h($currentScript."?".buildQuery(['page'=>1])).'">1</a></li>';
                      if ($start > 2) echo '<li class="page-item disabled"><span class="page-link">…</span></li>';
                    }
                    for ($p=$start; $p<=$end; $p++){
                      $active = ($p === $page) ? "active" : "";
                      echo '<li class="page-item '.$active.'"><a class="page-link" href="'.h($currentScript."?".buildQuery(['page'=>$p])).'">'.$p.'</a></li>';
                    }
                    if ($end < $pages) {
                      if ($end < $pages - 1) echo '<li class="page-item disabled"><span class="page-link">…</span></li>';
                      echo '<li class="page-item"><a class="page-link" href="'.h($currentScript."?".buildQuery(['page'=>$pages])).'">'.$pages.'</a></li>';
                    }
                  ?>

                  <li class="page-item <?php echo ($page >= $pages) ? "disabled" : ""; ?>">
                    <a class="page-link" href="<?php echo h($currentScript . "?" . buildQuery(['page'=>$next])); ?>">Next</a>
                  </li>
                </ul>
              </nav>
            <?php endif; ?>

          <?php endif; ?>
        <?php endif; ?>

      <?php endif; ?>
    </main>

  </div>
</div>
</body>
</html>
